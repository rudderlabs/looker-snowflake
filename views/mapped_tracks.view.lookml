view: mapped_tracks {
  derived_table: {
    ...
    sql: select *
        ,timestampdiff(minute,cast(lag(timestamp) over(partition by looker_visitor_id order by timestamp) as timestamp),cast(timestamp as timestamp)) as idle_time_minutes
      from (
